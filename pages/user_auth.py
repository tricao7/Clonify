...
if st.session_state.username != '':
    st.sidebar.write(
        f"You are logged in as {st.session_state.username.upper()}")
        ...
        if '' in [new_username, new_user_email, new_user_pas]:
            ...
            if user_db.find_one({'email': new_user_email}):
                ...
                if new_user_pas != user_pas_conf:
                    ...
                    user_db.insert_one(
                        {'log': new_username, 'email': new_user_email, 'pass': new_user_pas})
...
if st.session_state.username == "" and st.session_state.form != 'signup_form':
    signup_request = st.sidebar.button(
        'Create Account', on_click=select_signup)
